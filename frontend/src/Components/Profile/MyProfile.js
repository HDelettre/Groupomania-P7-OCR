...
const MyProfile = () => {
  ...
  useEffect(() => {
    if (updateProfile){
      dispatch(UPDATE_USER([user.story, `${user._id}`]))
  ...
  const [profilePict, setProfilePict] = useState(`${process.env.REACT_APP_API_IMG}/profile/${user.imageUrl}`);
  ...
  const validUpdateProfile = () => {
    ...
    async function fetchUpdateProfile() {
      try {
        const reponse = await fetch(`${process.env.REACT_APP_API_USER}/${user._id}`, {
          method: 'PUT',
          body: profileData,
      ...
      <div className="profilecontainer">
        <div className="profile">
          ...
          <div className="profile_row">
            ...
            <ProfileNavbar setProfilePict={setProfilePict} setProfileFile={setProfileFile} setMimeType={setMimeType} />
          </div>
          <div className="profile_valid" onClick={validUpdateProfile}>VALIDER</div>
